 ...
 * A1: Không. Vì B không gọi next() nên luồng xử lý bị ngắt tại đó, không chuyển tiếp sang các hàm phía sau.
 *
 * Q2: Nếu middleware gọi res.json() mà KHÔNG gọi next(),
 *     các middleware phía sau có chạy không?
 * A2: Không. Vì res.json() chỉ gửi response về client, không tự chuyển tiếp luồng đi nếu thiếu next().
 *
 * Q3: Error middleware có 4 tham số (err, req, res, next).
 *     Làm sao để "kích hoạt" error middleware từ route handler?
 * A3: Gọi next(err) và truyền đối tượng lỗi vào (ví dụ: next(new Error(...))).
 *
 * Q4: Middleware khai báo sau app.use("/api", router) có chạy cho route /api/users không?
 * A4: Không chạy đối với middleware thông thường (do route handler đã gửi response và ngắt luồng), nhưng sẽ chạy đối với Error middleware (nếu có lỗi xảy ra).
 *
 * Q5: Global middleware khai báo SAU route definition có áp dụng cho route đó không?
 * A5: Không. Express chạy theo thứ tự từ trên xuống, route khớp đã gửi response kết thúc request trước khi đi tới middleware phía dưới.
 *
...
function rateLimiter({ windowMs = 60000, max = 10 } = {}) {
  ...
  return (req, res, next) => {
    ...
    if (clientRecord.count <= max) {
      ...
      res.set("X-RateLimit-Remaining", max - clientRecord.count);
    ...
    res.status(429).json({ error: "Too Many Requests", retryAfter: clientRecord.resetTime - now });
  };
}

...
const seedProducts = [
  { name: "iPhone 15 Pro", price: 999, category: "tech", tags: ["apple", "smartphone", "new"], inStock: true, rating: 4.8 },
  { name: "Samsung Galaxy S24", price: 799, category: "tech", tags: ["samsung", "smartphone"], inStock: true, rating: 4.6 },
  { name: "MacBook Air M3", price: 1299, category: "tech", tags: ["apple", "laptop", "new"], inStock: false, rating: 4.9 },
  { name: "Office Chair Pro", price: 350, category: "furniture", tags: ["ergonomic", "office"], inStock: true, rating: 4.3 },
  { name: "Standing Desk", price: 650, category: "furniture", tags: ["office", "adjustable"], inStock: true, rating: 4.5 },
  { name: "Nike Running Shoes", price: 120, category: "sports", tags: ["nike", "running", "sale"], inStock: true, rating: 4.4 },
  { name: "Adidas Hoodie", price: 85, category: "clothing", tags: ["adidas", "casual", "sale"], inStock: true, rating: 4.2 },
  { name: "Instant Noodles Pack", price: 5, category: "food", tags: ["instant", "budget"], inStock: true, rating: 3.8 },
  { name: "Protein Powder 2kg", price: 65, category: "sports", tags: ["nutrition", "fitness"], inStock: false, rating: 4.1 },
  { name: "Gaming Keyboard", price: 180, category: "tech", tags: ["gaming", "mechanical"], inStock: true, rating: 4.7 },
];
...
async function filterByCategory(category) {
  ...
  console.log(products.map((product) => product.name).join(", "));
  return products;
}
...
async function getProductsPaginated({ page = 1, limit = 3, sort = "-createdAt", filter = {} } = {}) {
...
// Trả lời:
// - Nên dùng select() trong List API (lấy danh sách) để chỉ tải các trường cần hiển thị (như tên, giá, ảnh đại diện), giúp giảm dung lượng gói tin truyền qua mạng và tiết kiệm RAM/CPU.
// - Trong Detail API (chi tiết sản phẩm), chúng ta mới cần truy vấn toàn bộ thông tin của document.
...
async function compareLean() {
  ...
  console.log("With lean - typeof:", typeof withLean, "| constructor:", withLean.constructor.name);
  console.log("Without lean - typeof:", typeof withoutLean, "| constructor:", withoutLean.constructor.name);

  try {
    console.log("Calling withLean.save()...");
    await withLean.save();
  } catch (err) {
    console.log("Error calling withLean.save():", err.message);
  }

  try {
    console.log("Calling withoutLean.save()...");
    withoutLean.name = "Test Save Non-Lean";
    await withoutLean.save();
    console.log("Successfully saved withoutLean!");
  } catch (err) {
    console.log("Error calling withoutLean.save():", err.message);
  }
...
//     Trả lời: Mặc định aggregate() luôn trả về plain JavaScript objects (POJO) thay vì Mongoose Documents,
//     nên việc dùng .lean() là không cần thiết và không mang lại thay đổi gì.
//   - aggregate() và find() khác nhau về performance khi nào?
//     Trả lời:
//     + find() tối ưu cho các truy vấn đơn giản, lấy tài liệu trực tiếp theo điều kiện lọc và tận dụng index thông thường.
//     + aggregate() tối ưu khi cần gom nhóm (group), tính toán tổng hợp (sum/avg/max/min), hoặc liên kết dữ liệu (lookup) 
//       trên lượng dữ liệu lớn trực tiếp tại database engine, tránh việc kéo lượng lớn dữ liệu về ứng dụng Node.js để tính toán.
...
async function getCategoryStats() {
  ...
  stats.forEach((stat) => {
    console.log(
      `Category: ${stat._id.padEnd(8)} | count: ${stat.count} | avgPrice: $${stat.avgPrice.toFixed(2)} | maxPrice: $${stat.maxPrice}`
    );
  });

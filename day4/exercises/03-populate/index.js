...
async function seedData() {
  ...
  await Post.create([
    ...
    },
    {
      ...
      content: "Async/await is syntactic sugar on top of Promises. It makes asynchronous code look and behave more like synchronous code...",
      author: alice._id,
    ...
    },
    {
      ...
      content: "Both MongoDB and PostgreSQL are excellent databases. The choice depends on your data structure, query patterns, and team expertise...",
      author: bob._id,
    ...
    },
    {
      ...
      content: "This is a draft post about REST API design. Will cover versioning, error handling, pagination...",
      author: bob._id,
    ...
    },
    {
      ...
      content: "Middleware functions in Express are functions that have access to the request object (req), the response object (res), and the next function...",
      author: carol._id,
...
async function createPostWithReference() {
  ...
  const dave = await User.create({ name: "Dave", email: "dave@example.com", password: "Password123" });
  const post = await Post.create({ title: "Mongoose Populate Guide", content: "This is a post content.", author: dave._id });
  console.log("Before populate:", post.author);
  await post.populate("author");
  console.log("After populate:", post.author.name);
}
...
async function getPostsByUser(userId) {
  ...
  if (posts.length > 0 && posts[0].author) {
    userName = posts[0].author.name;
  } else {
    const user = await User.findById(userId).select("name").lean();
    if (user) userName = user.name;
  }

  console.log(`${userName} has ${posts.length} published posts:`);
  posts.forEach(post => {
    console.log(`  - ${post.title} (tags: ${post.tags ? post.tags.join(", ") : ""})`);
  });
}
...
async function getPublishedPosts() {
  ...
  posts.forEach(post => {
    ...
    grouped[authorName].posts.push(post.title);
  });

  for (const [authorName, info] of Object.entries(grouped)) {
    console.log(`Author: ${authorName} (${info.email})`);
    info.posts.forEach(title => {
      console.log(`  - ${title}`);
    });
  }
...
async function demonstrateVirtuals() {
  ...
  //
  const post = await Post.findOne({ published: true }).populate({ path: "author" });
  console.log("Snippet:", post.snippet);
  console.log("Display:", post.author.displayName);
  // Q1: populate() trong Mongoose tương đương với gì trong SQL?
  //     Có phải là 1 query hay nhiều query?
  //     A1: Tương đương với phép JOIN trong SQL. Dưới DB, Mongoose thực hiện nhiều query độc lập (thường là 2 query: 1 query lấy posts, gom các author ID, rồi chạy 1 query tìm users có ID thuộc danh sách đó) rồi map lại ở RAM Node.js.
  //
  // Q2: Khi nào nên embed document, khi nào nên dùng reference?
  //     Cho ví dụ: Comment (embed hay ref?)
  //     A2:
  //     - Embed: Khi dữ liệu con gắn liền với cha, kích thước nhỏ và ít đổi (Ví dụ: Address trong User). Với Comment, nếu bài viết ít comment (dưới vài chục) thì có thể embed.
  //     - Reference: Khi dữ liệu con lớn, tăng trưởng không giới hạn, hoặc cần query độc lập (Ví dụ: Post của User). Với Comment, nếu là hệ thống lớn có hàng ngàn comment mỗi post, nên dùng Reference để tránh vượt quá giới hạn 16MB của 1 document MongoDB.
  //
  // Q3: Nhược điểm của populate() về performance?
  //     Cách khắc phục trong production?
  //     A3:
  //     - Nhược điểm: Chậm vì thực hiện nhiều query độc lập và Mongoose phải dùng CPU để map dữ liệu thủ công. Dễ bị lỗi N+1 query nếu loop rồi populate.
  //     - Khắc phục: Chỉ select các trường cần thiết, sử dụng .lean() để bỏ bớt overhead của Mongoose Document, hoặc dùng Aggregation Framework ($lookup) để JOIN tối ưu trực tiếp dưới database level.
}

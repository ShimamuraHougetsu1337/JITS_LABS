...
app.get("/users/:id", async (req, res, next) => {
  try {
    ...
    // Trả lời: Vì findById chỉ làm nhiệm vụ truy vấn cơ sở dữ liệu. Việc không tìm thấy bản ghi có ID tương ứng không phải lỗi kỹ thuật của DB, mà chỉ là kết quả logic (không tồn tại). Mongoose trả về null để cho phép ứng dụng tự quyết định cách xử lý nghiệp vụ (ví dụ: tạo mới, bỏ qua hoặc báo lỗi 404).
    if (!user) {
...
app.use((err, req, res, next) => {
  ...
  // Mặc định che giấu thông tin lỗi 500 thực tế ở môi trường production
...
//
//     YOUR ANSWER: Vì err.message có thể chứa các chi tiết bảo mật thô như tên trường (fields), định dạng mong muốn, cấu hình validation trong schema (tiết lộ thiết kế hệ thống). Ngoài ra, các thông báo lỗi này mang tính kỹ thuật cao và không thân thiện với trải nghiệm của người dùng cuối.
//
// Q2: Khi nào dùng err.statusCode custom (throw từ service) vs Mongoose error?
//     Viết một ví dụ cho mỗi loại:
//
//     Custom statusCode error: Dùng khi kiểm tra các ràng buộc nghiệp vụ (business rules) của ứng dụng. Ví dụ: kiểm tra tài khoản có bị khóa không:
//                              `if (user.isBlocked) { const err = new Error("Blocked"); err.statusCode = 403; throw err; }`
//     Mongoose error: Xảy ra tự động ở tầng database khi thao tác vi phạm cấu hình của schema. Ví dụ: nhập sai định dạng email so với regex:
//                     `match: [/^\S+@\S+\.\S+$/, "Email is not valid"]` sẽ tự động ném ra ValidationError khi lưu.
//
// Q3: findByIdAndUpdate mặc định KHÔNG chạy Mongoose schema validation.
//     Cách làm cho nó validate: Bật tùy chọn `{ runValidators: true }` trong tham số thứ 3 (options) của hàm.
//     Tại sao đây không phải cách tốt nhất để update password?
//     Bởi vì các hàm update trực tiếp như findByIdAndUpdate chạy trực tiếp truy vấn xuống database và không kích hoạt pre("save") hook (nơi chứa code hash password tự động). Điều này sẽ làm mật khẩu mới bị lưu dưới dạng plain-text (chưa được mã hóa).
//
// Q4: Nếu user gửi id hợp lệ về format ObjectId (24 hex) nhưng không tồn tại
//     trong DB, Mongoose throw gì? Bạn phải xử lý ở đâu?
//
//     YOUR ANSWER: Mongoose sẽ KHÔNG ném ra lỗi nào cả, nó chỉ trả về giá trị null. Chúng ta cần tự kiểm tra kết quả null này trong route handler/controller và chủ động ném ra (throw) hoặc gọi next() với một lỗi custom 404 để xử lý.
//
// ─────────────────────────────────────────────────────────────────────────────

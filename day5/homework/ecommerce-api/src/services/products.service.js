...
async function getProducts(filters = {}, options = {}) {
  ...
  if (options.sort) {
    // If sort is passed as a string like "price_asc", convert to mongoose format
    if (options.sort === "price_asc") sort = { price: 1 };
    else if (options.sort === "price_desc") sort = { price: -1 };
...
async function addRating(id, userId, ratingData) {
  // By passing isActive: true, we override the pre-find hook's undefined check
  ...
  if (existingIndex > -1) {
    ...
    if (ratingData.review !== undefined) {
  ...
  const result = await Product.aggregate([
    ...
    { $unwind: { path: "$ratings", preserveNullAndEmptyArrays: true } },
    {
      $group: {
        ...
        avgRating: { $avg: "$ratings.score" },
        totalRatings: { $sum: { $cond: [{ $ifNull: ["$ratings", false] }, 1, 0] } },
        score1: { $sum: { $cond: [{ $eq: ["$ratings.score", 1] }, 1, 0] } },
        score2: { $sum: { $cond: [{ $eq: ["$ratings.score", 2] }, 1, 0] } },
        score3: { $sum: { $cond: [{ $eq: ["$ratings.score", 3] }, 1, 0] } },
        score4: { $sum: { $cond: [{ $eq: ["$ratings.score", 4] }, 1, 0] } },
        score5: { $sum: { $cond: [{ $eq: ["$ratings.score", 5] }, 1, 0] } }
      }
    },
    {
      $project: {
